...
static void nativeInit(JNIEnv *env, jobject thiz) {
    ...
    if (!Charset.self) {
        ...
        MainActivity.self = FindClassOrDie(env,  "com/clandro/x11/MainActivity");
        MainActivity.getInstance = FindMethodOrDie(env, MainActivity.self, "getInstance", "()Lcom/clandro/x11/MainActivity;", JNI_TRUE);
...
JNIEXPORT jint JNI_OnLoad(JavaVM *vm, __unused void *reserved) {
    ...
    jclass cls = (*env)->FindClass(env, "com/clandro/x11/LorieView");
...
__attribute__((constructor)) static void init(void) {
    pthread_t t;
    if (!strcmp(__progname, "com.clandro.x11"))
